...
static ParityLookup parity_lookup[128];
static int          encode_state;

#define NR_CONVOLUTIONAL_STATES 64
#define NORMAL_PATH_LENGTH      105
#define ALTERNATE_1_PATH_LENGTH   NORMAL_PATH_LENGTH-4
#define ALTERNATE_2_PATH_LENGTH   NORMAL_PATH_LENGTH-6

static float        acm[NR_CONVOLUTIONAL_STATES]; /* Accumulated distance */
static int          path[NR_CONVOLUTIONAL_STATES][NORMAL_PATH_LENGTH];
static int          hp;
static int          path_length;

#define BF(v,w,x,y,z) tempa=(acm[w]*0.999)+(metric[x]*0.001);tempb=(acm[y]*0.999)+(metric[z]*0.001);if(tempa>tempb){tm[v]=tempa;path[v][hp]=w;}else{tm[v]=tempb;path[v][hp]=y;}

static void parity( int state, int *bit1, int *bit2 )
{
	int count;
