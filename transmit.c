/*
 *
 * Experimental Digital Voice modem.
 *
 *
 */
#include <unistd.h>
...
void tx_symbol( FComplex symbol )
{
	...
	for( k = 0; k < 4; k++ )
	{
		...
		sample  =  cos(acc)*output.real;	
		sample -=  sin(acc)*output.imag;	
		sample *= 2.0f;   //make the transmit audio louder
		acc += (2*PI*(CENTER_FREQUENCY)/SAMPLE_RATE);	
